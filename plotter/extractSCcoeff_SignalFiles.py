...
def main():
    ...
    if len(sys.argv) < 2:
        print "Usage: python extractCoeff.py <input_file1> [<input_file2> ...]"
        return

    input_files = sys.argv[1:]

    output_filename = "extractedCoefficients_multiFiles.txt"
        ...
        for hist_name, multiplier in coeff_multipliers.iteritems():
            ...
            for idx, input_file in enumerate(input_files):
                ...
                if not root_file or root_file.IsZombie():
                    print "Error: Could not open the ROOT file: %s" % input_file
                    continue
                histogram = root_file.Get(hist_name)
                if not histogram:
                    print "Error: Histogram '%s' not found in %s." % (hist_name, input_file)
                    root_file.Close()
                    continue
                if summed_hist is None:
                    summed_hist = histogram.Clone("summed_" + hist_name)
                    summed_hist.SetDirectory(0)
                else:
                    summed_hist.Add(histogram)
                root_file.Close()
            if summed_hist is None:
                print "Error: Histogram '%s' not found in any input file." % hist_name
                continue

            fb_asymmetry, fb_asymmetry_uncertainty, coefficient, coefficient_uncertainty = compute_fb_asymmetry(summed_hist, multiplier)
