...
class ZprimePreselectionModule_Visibel : public ModuleBASE {

public:
  explicit ZprimePreselectionModule_Visibel(uhh2::Context&);
...
protected:
  ...
  std::unique_ptr<uhh2::Selection> vis_sel;
  std::unique_ptr<uhh2::Selection> full_sel;
...
void ZprimePreselectionModule_Visibel::book_histograms(uhh2::Context& ctx, vector<string> tags){
...
void ZprimePreselectionModule_Visibel::fill_histograms(uhh2::Event& event, string tag){
  string mytag = tag+"_General";
...
void ZprimePreselectionModule_Visibel::init_JEC_JLC(uhh2::Context& ctx){
  ...
  if(is2016v2 || is2016v3){
    cout<<"ZprimePreselectionModule_Visibel uses JEC for 2016 data/MC"<<endl;
  ...
  if(is2017){
    cout<<"ZprimePreselectionModule_Visibel uses JEC for 2017 data/MC"<<endl;
  ...
  if(is2018){
    cout<<"ZprimePreselectionModule_Visibel uses JEC for 2018 data/MC"<<endl;
...
ZprimePreselectionModule_Visibel::ZprimePreselectionModule_Visibel(uhh2::Context& ctx){
  ...
  vis_sel.reset(new TTbarSemiLepVisibleSelection());
  full_sel.reset(new TTbarSemiLepFullPS());

  // Book histograms
  vector<string> histogram_tags = {"Input","FullPS","VisiblePS","Lumiselection", "Metfilters", "Lepton1", "JetID", "JetCleaner1", "JetCleaner2", "TopjetCleaner", "Jet1", "Jet2", "MET"};
...
bool ZprimePreselectionModule_Visibel::process(uhh2::Event& event){


  //  cout<<"Getting started... "<<event.event<<endl;
  fill_histograms(event, "Input");

  const bool pass_full = full_sel->passes(event);
  if(!pass_full) return false;
  fill_histograms(event, "FullPS");
 
  // Visible selection

  const bool pass_vis = vis_sel->passes(event);
  if(!pass_vis) return false;
  fill_histograms(event, "VisiblePS");
/*
  // Lumi selection
  if(event.isRealData){
  ...
  if(event.isRealData){
    ...
    if(apply_A+apply_B+apply_C+apply_D+apply_E+apply_F+apply_G+apply_H != 1) throw std::runtime_error("In ZprimePreselectionModule_Visibel.cxx: Sum of apply_* when applying JECs is not == 1. Fix this.");
...
UHH2_REGISTER_ANALYSIS_MODULE(ZprimePreselectionModule_Visibel)
