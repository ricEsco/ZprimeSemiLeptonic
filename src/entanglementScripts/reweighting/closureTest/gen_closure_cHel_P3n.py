...
#   does the f=+1 reweight  w = 1 / (1 + lambda*O)  make the slope of the GEN cHel_P3n distribution
#   VANISH?  We measure it directly on the inclusive gen sample the map was built from.
#
...
def main(skim=SKIM, mapf=MAPF, obs=OBS):
    ...
    ################################## sequestering original code ##################################
    # out_pdf = "gen_closure_%s.pdf" % obs
    ################################## sequestering original code ##################################


    ######################## testing closure outside of SR, truly inclusive ########################
    out_pdf = "FullInclusive_gen_closure_%s.pdf" % obs
    ######################## testing closure outside of SR, truly inclusive ########################
    ...
    t = fin.Get(TREE)                      # reco skim tree...
    if not t:                              # ...or the inclusive-gen dump ("gendump") -- take first TTree
        for key in fin.GetListOfKeys():
            o = key.ReadObj()
            if o.InheritsFrom("TTree"):
                t = o; break
    print("tree: %s (%d entries)" % (t.GetName(), t.GetEntries()))
...
def _plot(h_nom, h_up, h_dn, out_pdf, obs, A_nom, A_up2, A_up5):
    ...
    h_nom.SetTitle("gen %s reweighting closure (inclusive gen);gen %s;a.u. (unit area)" % (obs, obs))
