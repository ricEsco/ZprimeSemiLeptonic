...
class TTbarGenPy(object):
    ...
    (e_had,                                         #  0  = hadronic
     e_ehad, e_muhad, e_tauhad,                     # 1-3 = semileptonic
     e_ee, e_mumu,e_tautau, e_emu, e_etau, e_mutau, # 4-9 = dileptonic
     e_notfound) = range(11)
    ...
    def _has_top_mother(self, gp, top):
        m1 = gp.mother(self.genparticles, 1)
        m2 = gp.mother(self.genparticles, 2)
        return (bool(m1) and m1.index() == top.index()) or \
               (bool(m2) and m2.index() == top.index())

    def _build(self, throw):
        genparticles = self.genparticles
        n_top = n_antitop = 0
        top = antitop = None

        # loop over all genparticles
        for i in range(genparticles.size()):
            genp = genparticles[i]
            ...
            # assuming the top quark has 2 daughters, find the W and b using the daughter() method
            w = genp.daughter(genparticles, 1)
            b = genp.daughter(genparticles, 2)
            if (not w) or (not b):
            ...
            ### in case of FSR or PS QED radiation, the W or b may not be direct daughters of the top ###
            ###                        find daughters using mother() method instead                   ###
            #############################################################################################
            # W boson daughter still not found
            if abs(w.pdgId()) != 24: 
                for j in range(genparticles.size()):
                    gp = genparticles[j]
                    if abs(gp.pdgId()) == 24 and self._has_top_mother(gp, genp):
                        w = gp; break
            # b quark daughter still not found
            ...
            # follow W boson daughters until we find the final decay products (2 daughters)
            ...
            while n_wdau != 2:
                if wd1 and not wd2:
                    w = wd1
                    wd1 = w.daughter(genparticles, 1); wd2 = w.daughter(genparticles, 2)
                elif wd1 and wd2:
                    n_wdau = 2
                else:
                    if throw: raise RuntimeError("W has no daughters")
                    return False
            ############################ ttbar system has been found ############################

            # fill member variables according to top quark charge
            if genp.pdgId() == 6:
        ...
        # number of charged leptons in W daughters gives the decay channel
        n_e = n_m = n_t = 0
    ...
    # accessors to the top, antitop, and their daughters
    ...
    # accessors for top and b quarks on hadronic/leptonic side of decay
